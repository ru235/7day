# service555 — 8 статичных сервисов на Astro
...
| S08 | domenpodbor.ru | Подбор доменов |

Сборка локально: `npm run build:all` → `apps/*/dist` → заливка на reg.ru по FTP (`npm run deploy` читает `.env`).
Дизайн: impeccable, единый DESIGN.md. SEO: classic + AI-поиск (llms.txt).
